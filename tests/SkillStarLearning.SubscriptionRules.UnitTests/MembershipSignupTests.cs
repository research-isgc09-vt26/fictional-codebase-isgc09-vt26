...
namespace SkillStarLearning.SubscriptionRules.UnitTests
{
    ...
    public sealed class MembershipSignupTests
    {
        ...
        [TestMethod]
        public async Task SignupForSegmentationB_DoesNotIncludeFreeTrial()
        {
            var now = new DateTimeOffset(2026, 5, 14, 10, 0, 0, TimeSpan.Zero);
            var service = TestFactory.CreateMembershipSignupService(new FixedTimeProvider(now));
            ...
            Assert.AreEqual(now.UtcDateTime, result.TrialStartsOn);
            Assert.AreEqual(now.UtcDateTime, result.TrialEndsOn);
        }

        [TestMethod]
        public async Task SignupForSegmentationB_UsesMarketSpecificMessage()
        {
            ...
            StringAssert.Contains(result.CustomerMessage, "Your subscription is registered");
        }
    }
